...
list_orig_case=sorted(set([i for i in os.listdir("testcase_modules/testcase_input") if int(i.split('.')[0])<1000]),key=lambda x: int(x.split('.')[0]))
...
def create_1000():
    ...
    for orig_case in tqdm(list_orig_case,"generate 1000 series"):
        f_in=open(os.path.join("testcase_modules/testcase_input",orig_case),"r")
        temp_data=f_in.read()
        temp_data=temp_data.replace("high",'high2').replace("low","low2")
        case_number=int(orig_case.split('.')[0])
        f_out=open(os.path.join("testcase_modules/testcase_input",f"{1000+case_number}.in"),"w")
...
def create_2000():
    ...
    for orig_case in tqdm(list_orig_case,"generate 2000 series"):
        ...
        case_number=int(orig_case.split('.')[0])
        f_out=open(os.path.join("testcase_modules/testcase_input",f"{2000+case_number}.in"),"w")
        f_out.write(temp_data)
        f_in.close()
        f_out.close()
        # break;
...
def create_3000():
    ...
    for orig_case in tqdm(list_orig_case,"generate 2000 series"):
        ...
        case_number=int(orig_case.split('.')[0])
        f_out=open(os.path.join("testcase_modules/testcase_input",f"{3000+case_number}.in"),"w")
        f_out.write(temp_data)
        f_in.close()
        f_out.close()
        # break;
