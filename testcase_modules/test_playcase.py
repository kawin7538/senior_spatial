...
class TestPlayCase:
    ...
    def _play_case(self):
        for precision in [100,99,95,90,75,50]:
            for file_name in tqdm(sorted(self.list_file_name),desc=f"Precision{precision}"):
                ...
                dataloading_obj=TestDataLoading(geopackage_obj,input_value[1],precision)
    ...
    def _plot_local_gi(self,dataloading_obj:TestDataLoading, geopackage_obj:TestGEOPackage,file_name):
        ...
        global_g_df.to_csv(os.path.join("output/testcase_output",file_name.split('.')[0]+".g.global.csv"),index=False)
    ...
    def _plot_moran(self,dataloading_obj:TestDataLoading, geopackage_obj:TestGEOPackage,file_name):
        ...
        global_moran_df.to_csv(os.path.join("output/testcase_output",file_name.split('.')[0]+".moran.global.csv"),index=False)
    ...
    def _plot_geary(self,dataloading_obj:TestDataLoading, geopackage_obj:TestGEOPackage,file_name):
        c_global=Geary(dataloading_obj.map_with_data['value'],geopackage_obj.get_weight(),transformation="B",permutations=9999)
        global_c_df=pd.DataFrame(columns=['c_value','c_z_sim','c_p_z_sim'])
        global_c_df.loc[len(global_c_df)]=[c_global.C,c_global.z_sim,c_global.p_z_sim]
        global_c_df.to_csv(os.path.join("output/testcase_output",file_name.split('.')[0]+".c.global.csv"),index=False)

        c_local=Geary_Local(geopackage_obj.get_weight(), labels=True, sig=0.1, permutations=9999, keep_simulations=False)
        ...
        local_c_df['c_label']=local_c_df['c_label'].astype(int).replace({1:'outlier',2:'cluster',3:'other',4:'not-significant'})
        local_c_df.to_csv(os.path.join("output/testcase_output",file_name.split('.')[0]+".c.local.csv"),index=False)

        fig,ax=plt.subplots(1,figsize=(12,12))
        spots=['not-significant','other','cluster','outlier']
        color_list=['lightgrey','lightblue','red','orange'][::-1]
        hmap=colors.ListedColormap(color_list)
        geopackage_obj.map.assign(c_label=local_c_df['c_label']).plot(column='c_label',categorical=True,linewidth=1,ax=ax,edgecolor='black',legend=True,cmap=hmap,k=4,categories=spots[::-1])
        ax.set_axis_off()
        plt.savefig(os.path.join("output/testcase_output",file_name.split('.')[0]+".c.png"),dpi=300,bbox_inches='tight')
        plt.close('all')